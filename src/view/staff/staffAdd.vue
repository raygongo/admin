	...
	<div>
		<div class="top-container">
			<div class="breadcrumb-container">
				<mis-breadcrumb 
					separator="-" 
					:attr="[{name: '员工管理', to: '/staffList'}, {name: '添加员工'}]"></mis-breadcrumb>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			this.getBrand('factory', this.API.getFactoryBrand)
			this.getBrand('engine', this.API.getEngineBrand)
			this.getBrand('trans', this.API.getTransBrand)
		},

		methods: {
			getBrand(key, api) {
				this.$http.get(api).then(res => {
					this.brandFilter(key, res.data)
				})
			},
			...
			uploadSuccess(res, file) {
				if (res.status === 1) this.staffInfo.headerImg = res.data[0].url
			},
			...
			handleSubmit() {
				this.$refs.form.validate(valid => {
					if (valid) {
						...
						this.$http.post(this.API.staffAdd, params).then(res => {
